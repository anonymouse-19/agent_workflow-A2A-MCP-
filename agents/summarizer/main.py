"""
Summarizer Agent — FastAPI microservice (port 8003).

Creates summaries and compiles final clinical reports.
Produces structured clinical briefs via Groq LLM.
...
app = FastAPI(title="Summarizer Agent", lifespan=lifespan)
...
async def agent_card():
    with open(AGENT_CARD_PATH) as f:
        card = json.load(f)
    card["url"] = os.environ.get("SUMMARIZER_AGENT_URL", card.get("url", "http://localhost:8003"))
    return JSONResponse(card)


def _assemble_text(context: dict) -> str:
    """Assemble all readable text from context."""
    texts = []
...
async def tasks_send(request: Request):
    """A2A task endpoint — produce a summary or clinical brief."""
    body = await request.json()
    task_id = body.get("id", str(uuid.uuid4()))
    correlation_id = body.get("correlation_id", str(uuid.uuid4()))
    task_input = body.get("input", {})

    tool_name = task_input.get("tool", "summarize_text")
    context = task_input.get("context", {})
    analysis = task_input.get("analysis", {})

    logger.info(f"[{correlation_id[:8]}] Summarizer task: tool={tool_name}")

    try:
        text = _assemble_text(context) or task_input.get("text", "")
        result = {}
        ...
        if not text and not analysis:
            return JSONResponse({
                ...
                "status": "completed",
                "correlation_id": correlation_id,
                "output": {"summary": "(No content to summarize)", "method": "none"},
                "error": None,
            })

        # Step 1: Extractive summary via MCP tool
        if text:
            summary_result = await _mcp_call("summarize_text", {"text": text})
            result["summary"] = summary_result.get("summary", "")
            result["method"] = summary_result.get("method", "")
            result["sentences_selected"] = summary_result.get("sentences_selected", 0)

        # Step 2: Include analysis results
        if analysis:
            result["analysis_included"] = list(analysis.keys())
            result["analysis_details"] = analysis

        # Step 3: LLM-enhanced clinical brief
        brief_input = result.get("summary", "")
        if analysis:
            brief_input += "\n\nAnalysis results:\n" + json.dumps(analysis, default=str)[:2000]
        ...
        try:
            llm_result = await _mcp_call(
                ...
                {
                    "text": brief_input,
                    "context": "Healthcare clinical data summary and analysis",
                    "task_type": "enhance",
                },
            )
            if "error" not in llm_result:
                result["clinical_brief"] = llm_result.get("enhanced_text", "")
                result["groq_model"] = llm_result.get("model", "")
                result["tokens_in"] = llm_result.get("input_tokens", 0)
                result["tokens_out"] = llm_result.get("output_tokens", 0)
        except Exception as e:
...
async def tasks_stream(request: Request):
    ...
    async def event_stream():
        yield f"data: {json.dumps({'id': task_id, 'status': 'in-progress', 'agent': 'summarizer', 'tool': 'summarize_text'})}\n\n"
        try:
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("SUMMARIZER_PORT", "8003"))
